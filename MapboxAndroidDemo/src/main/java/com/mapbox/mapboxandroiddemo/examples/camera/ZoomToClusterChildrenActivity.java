...
/**
 * Tap on a cluster and adjust the map camera bounds to fit that cluster's data.
 */
public class ZoomToClusterChildrenActivity extends AppCompatActivity implements MapboxMap.OnMapClickListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.activity_zoom_to_cluster_children);
      ...
      public void onMapReady(@NonNull MapboxMap map) {
        map.setStyle(Style.DARK, new Style.OnStyleLoaded() {
          ...
          public void onStyleLoaded(@NonNull Style style) {
            ...
            mapboxMap.addOnMapClickListener(ZoomToClusterChildrenActivity.this);

            Toast.makeText(ZoomToClusterChildrenActivity.this,
  ...
  public boolean onMapClick(@NonNull LatLng point) {
    ...
    if (mapClickFeatureList.size() > 0) {
      FeatureCollection clusterChildrenFeatureCollection = geoJsonSource.getClusterLeaves(mapClickFeatureList.get(0),
        8000, 0);
      moveCameraToChildrenBounds(clusterChildrenFeatureCollection);
    }

    return true;
  }

  private void moveCameraToChildrenBounds(FeatureCollection featureCollectionToInspect) {
